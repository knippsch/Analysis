...
#  fitfunc1 = lambda p,t: p[0]*(np.exp(-t*p[1]) + np.exp(-(T-t)*p[1]))
#  fitfunc2 = lambda p,t: p[0]
#  counter = 0
##  for lo in range(10, C2.shape[1]):
##    for up in range(C2.shape[1], 10, -1):
#  for lo in range(15, 16):
#    for up in range(C2.shape[1], C2.shape[1]-1, -1):
...
#      res_C2, chi2, pvalue = fit.fitting(fitfunc1, t, C2[:,lo:up], \
#                                         start_parm, correlated = 0)
...
#    i += 1
#
#  fitfunc1 = lambda p,t: p[0]*(np.exp(-t*p[1]) + np.exp(-(T-t)*p[1])) + p[2]
#  counter = 0
##  for lo in range(10, C4.shape[1]):
##    for up in range(C4.shape[1], 10, -1):
#  for lo in range(15, 16):
#    for up in range(C4.shape[1], 23, -1):
...
#    i += 1
#
#  fitfunc1 = lambda p,t: -p[0]*np.exp(-p[1]*(t+T+1.)) * \
#                         (-1.+np.exp(p[1])) * \
#                         (np.exp(p[1]+2.*p[1]*t)-np.exp(p[1]*T))
#  fitfunc2 = lambda p,t: p[0]
#  counter = 0
#  for lo in range(14, C4.shape[1]):
#    for up in range(C4.shape[1], 14, -1):
#      if (up-lo-2) < 2: # only fits with dof>1 are ok
#        continue
#      t = np.asarray(tt[lo:up])
#      print "\nResult from bootstrapsample fit in interval (2pt):", lo, up-1
